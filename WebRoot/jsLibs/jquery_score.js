...
jQuery.fn.rater	= function(options) {
	...
	var settings = {
		...
		title_format	: null,
		info_format	: null,
		image	: '../images/default/base/stars.jpg',
		imageAll :'../images/default/base/stars-all.gif',
		defaultTips :true,
		clickTips :true,
		width	: 24,
		height	: 24
	...
	content.find('.rater-star-item').mouseover(function() {
		...
		if (typeof settings.title_format == 'function') {
			jQuery(this).parents(".rate-comm").next('.rater-star-result').find(".rater-star-result-text").html(current_value+'分&nbsp;'+settings.title_format(current_value));
			jQuery(this).parents(".rate-comm").next('.rater-star-result').find(".rater-star-result-input").find("input").val(current_value);
		}
...
$(function(){
	...
	info_format	: function(value) {
		...
		switch (value) {
			case 1 : 
				info	= '<div class="info-box">1分&nbsp;很不满意<di</div></div>';
				break;
			case 2 : 
				info	= '<div class="info-box">2分&nbsp;不满意<div></div></div>';
				break;
			case 3 : 
				info	= '<div class="info-box">3分&nbsp;一般<div></div></div>';
				break;
			case 4 : 
				info	= '<div class="info-box">4分&nbsp;满意<div></div></div>';
				break;
			case 5 : 
				info	= '<div class="info-box">5分&nbsp;非常满意<div></div></div>';
				break;
			default :
				info = value;
				break;
	...
	$('#rate-comm-4').rater(options);
	$('#rate-comm-5').rater(options);
	$('#rate-comm-6').rater(options);
});