...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.check==true){
		str+= '<input type="checkbox" name="treeCk" class="cx" id="c'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';
		//alert(str);
	}
	if (node.url) {
...
function checkAll(){
	$("input[name='treeCk']").attr("checked",true);
}
	
function cancelAll(){
	$("input[name='treeCk']").removeAttr("checked");
}